...
# ----------------- methods ---------------------
def test_by_betting(seq1, seq2, R=250.0, C=15.0, epsilon=0.01):
    """
    Two-sided epsilon-fairness test by betting.
    Runs two ONS-betting martingales:
        M_plus  tests  risk1 - risk2 >  ε
        M_minus tests  risk2 - risk1 >  ε
    Split initial wealth 0.5 each. Reject if either exceeds 1/alpha.
    """
    const = 3 / (2 - np.log(3))
    lambd = 0
    zt2 = 0
    ...
    for t in range(1, T):
        diff = max(seq1[t] - seq2[t], seq2[t] - seq1[t])
        # martingale for risk1 - risk2 > ε
        g = epsilon - diff 
        St = 1 + lambd * g
        wealth = wealth*St
        z = g / (1 + lambd * g)
        zt2 += z ** 2
        lambd = lambd + const * z / (1 + zt2)
        lambd = np.clip(lambd, 0.0, 0.5)
        wealth_hist.append(min(wealth,R))
        
        """
        print("wealth: ",f'{wealth:.2f}', 
            "lambda: ",f'{lambd:.2f}', 
            "payoffs: ",f'{g:.2f}',
            "diff: ",f'{diff:.2f}',
            "seq_1: ",seq1[t],
...
def betting_experiment(seq1, seq2, alphas, epsilon=0.01, iters=10):
    results, rejections = [], []
    for _ in range(iters):
        ...
        taus, rejects = [], []
        for alpha in alphas:
            w, decision = test_by_betting(seq1, seq2, alpha=alpha, epsilon=epsilon)
            taus.append(len(w))
            rejects.append(decision == "reject")
        results.append(taus)
